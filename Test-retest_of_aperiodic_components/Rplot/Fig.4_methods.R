...
df3 <- data_summary(filtered_data, varname="ICC_value", 
                    groupnames=c("state", "methods"))


# Histogram plot
p <- ggplot(df3, aes(x=state, y=ICC_value, fill=methods)) + 
...
pwc_methods <- filtered_data %>%
  group_by(state) %>%
  pairwise_t_test(ICC_value ~ methods, paired = TRUE, p.adjust.method = "bonferroni") 
